...
To get started, lets get http://onlinelibrary.wiley.com/doi/10.1111/geb.12569/epdf`sf installed:
...
And to plot just a single field:
...
That's nice, but if we want to interact with the data we can't do that just yet (I'm planning to add `sf` support to [`quickmapr`](https://cran.r-project.org/package=quickmapr), but haven't yet).  But, we can do that with `mapview` (currently only in the development version)
